...
```{python}
def video_level_feats(model, device, dataset, layers, diff_layer, frame_size, center_crop,
    ...
    videos_feats = videoset_frame_feats(model, device, video_list, video_path, transform,
                                         dataset, layers=layers)
    # Pool the frame level features to get video level features
    pooled_features = pooling.simple_pooling(videos_feats, pool_type=pool_type)
    
    if frame_diff:
        diff_feats = videoset_frame_feats(model, device, video_list, video_path, transform,
                                           dataset, layers=diff_layer, frame_diff=frame_diff)
        # Pool the frame level features to get video level features
        pooled_diff = pooling.simple_pooling(diff_feats, pool_type=pool_type)
...
def init_vqa(model_name, vqa_dataset, cross_dataset=None):
    ...
    model = sfv.set_sf_model(device, fine_tune=False, model_name=model_name,
                             frame_size=frame_size, center_crop=center_crop)
                
    return model, device, vqa_dataset, cross_dataset, frame_size, center_crop, layers, diff_layer, frame_diff
...
def main():
    ...
    '''
    # Initialize vqa
    model, device, vqa_dataset, cross_dataset, frame_size, center_crop, layers,\
                                    diff_layer, frame_diff = init_vqa('inceptionv3',
                                                                      'konvid1k')
    # Get video level features by pooling frame level features
    pooled_feats, scores = video_level_feats(model, device, vqa_dataset, layers, diff_layer,
                                             frame_size, center_crop, frame_diff)
    
    if cross_dataset == 'live':
        # Get video level features by pooling features of consecutive frames' differences
        c_pooled_feats, c_scores = video_level_feats(model, device, cross_dataset, layers, diff_layer,
                                             frame_size, center_crop, frame_diff)
    else:
