...
def get_videoset_features() -> list:
    ...
    # Conv layers from which feature maps are extracted and convertd to gram matrices
    layers = {
              '0': 'conv1_1',
              # '5': 'conv2_1', 
              # '10': 'conv3_1', 
    ...
    for seq in video_list:
        ...
        frames_features = np.array(sfv.get_video_style_features(vid_frames, model, layers, device, transform))
...
def main(mode):
    ...
    if mode == 'save':
        load_save_features('features.npy', 'scores.npy', features=videos_feats, scores=dmos, mode=mode)
    elif mode == 'load':
        videos_feats, dmos = load_save_features('features.npy', 'scores.npy', mode=mode)
